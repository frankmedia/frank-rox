-- Delete Frank's current active plan (clientId 25)
-- This will allow regenerating a fresh programme with all the latest fixes
...
WHERE block_id IN (
  ...
  WHERE p.client_id = 25 AND p.status = 'active'
);
...
WHERE plan_id IN (
  SELECT id FROM plans WHERE client_id = 25 AND status = 'active'
);

-- Delete the plan itself
DELETE FROM plans WHERE client_id = 25 AND status = 'active';

-- Verify deletion
SELECT 
  'Plans deleted successfully' as status,
